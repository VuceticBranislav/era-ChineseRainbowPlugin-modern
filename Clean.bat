...
for /f "delims=" %%a in ('dir /B /S *.bpl, *.bpi, *.dcp, *.so, *.apk, *.drc, *.map, *.dres, *.rsm, *.tds, *.dcu, *.lib, *.a, *.o, *.ocx, *.res  2^>nul') do del "%%a"

for /f "delims=" %%a in ('dir /B /S *.exe, *.dll 2^>nul') do (
  if not "%%~dpa"=="%cd%\LocaleEmulator\" (del "%%a" ))
